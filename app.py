...
class ResumeApp:
    ...
    def apply_global_styles(self):
        # Apply custom interactive cursor effect
        components.html(
            """
            <script>
                const doc = window.parent.document;
                
                // Only create if we haven't already injected it
                if (!doc.getElementById('custom-cursor-dot')) {
                    // Inject CSS
            ...
            <div style='display: flex; justify-content: center; align-items: center; margin-bottom: 10px;'>
                <a href='#' target='_blank' style='text-decoration: none;'>
                    <div style='display: flex; align-items: center; background-color: #24292e; padding: 5px 10px; border-radius: 5px; transition: all 0.3s ease;'>
                        <span style='color: white; font-size: 14px;'>Developed with ❤️ by Kranthikumar Veggalam</span>
                    </div>
                </a>
            </div>
            """, unsafe_allow_html=True)
    ...
    "Specific Role", roles, key="standard_role")
            ...
            st.markdown(f"""
            <div class="glass-card" style='padding: 20px; margin: 10px 0; border-left: 3px solid var(--accent-secondary);'>
                <h3>{selected_role}</h3>
        ...
        analysis['document_type']} document, not a resume!")
                    ...
                    with col2:
                        ...
                        if analysis.get('contact_suggestions'):
                                st.markdown("""
                                <div class="glass-card" style='padding: 15px; margin: 10px 0; border-left: 3px solid var(--accent-primary);'>
    ...
    def render_home(self):
        ...
        with col1:
            ...
            hero_section(
                ...
                "Transform your career trajectory with ultra-advanced AI analysis. Our premium engine provides deep insights, formats your resume for ATS success, and gives actionable recommendations."
            )
            ...
            with col_btn:
                if st.button("🚀 IGNITE YOUR CAREER", key="get_started_btn", type="primary", use_container_width=True):
                    st.session_state.page = "resume_analyzer"
                    st.rerun()
                    
        with col2:
            # Using a public resume/AI Lottie animation URL
            lottie_url = "https://assets5.lottiefiles.com/packages/lf20_vnikrcia.json"
            animation = self.load_lottie_url(lottie_url)
            if animation:
                st_lottie(animation, height=400, key="hero_lottie")
            else:
                st.markdown("""
                    <div class="glass-card" style="height: 350px; display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-brain icon-pulse glow-text" style="font-size: 8rem;"></i>
                    </div>
                """, unsafe_allow_html=True)

        st.markdown("<hr style='border: 1px solid var(--border-glass); margin: 3rem 0;'>", unsafe_allow_html=True)
        
        # Features Section Title
        st.markdown("<h2 class='glow-text' style='text-align: center; font-size: 2.5rem; margin-bottom: 2rem;'>Premium Features</h2>", unsafe_allow_html=True)
        ...
        with feat_col1:
            feature_card(
                "fas fa-robot",
                "AI-Powered Analysis",
                "Get instant feedback with our Gemini-powered engine that identifies strengths and critical blindspots."
            )
        ...
        with feat_col2:
            feature_card(
                "fas fa-magic",
                "Smart Builder",
                "Create ATS-optimized resumes instantly with our intelligent builder that formats everything flawlessly."
            )
        ...
        with feat_col3:
            feature_card(
                "fas fa-chart-line",
                "Deep Insights",
                "Access granular analytics and personalized career roadmaps to outshine the competition."
            )
    ...
    def main(self):
        ...
        # Navigation mapping
        import re
        original_page_names = list(self.pages.keys())
        
        # Create clean display names (no emojis)
        display_names = []
        display_to_original = {}
        for name in original_page_names:
            clean_name = re.sub(r'[^\x00-\x7F]+', '', name).strip()
            display_names.append(clean_name)
            display_to_original[clean_name] = name
            
        icons_mapping = {
            "HOME": "house",
            "RESUME ANALYZER": "search",
            "RESUME BUILDER": "file-earmark-text",
            "DASHBOARD": "bar-chart",
        ...
        with st.sidebar:
            st.markdown("""
                <div style="text-align: center; margin-bottom: 2rem;">
                    <h2 class="glow-text" style="font-size: 2.2rem; margin: 0;">Smart AI</h2>
                    <h3 style="font-size: 1.2rem; margin: 0; color: var(--accent-secondary);">Resume Analyzer</h3>
                </div>
            """, unsafe_allow_html=True)
            
            # Map current page state back to index for option menu
            current_page = st.session_state.get('page', 'home')
            page_mapping_reverse = {name.lower().replace(" ", "_"): i 
                           for i, name in enumerate(display_names)}
            
            nav_index = page_mapping_reverse.get(current_page, 0)
            
            # Use streamlit-option-menu with upgraded premium CSS
            selected = option_menu(
                menu_title=None,
                options=display_names,
                icons=[icons_mapping.get(p, "circle") for p in display_names],
                menu_icon="cast",
                default_index=nav_index,
                styles={
                    "container": {"padding": "0!important", "background-color": "transparent"},
                    "icon": {"color": "#00d2ff", "font-size": "1.1rem", "margin-right": "8px"}, 
                    "nav-link": {
                        "color": "#ffffff",
                        "font-size": "0.95rem", 
                        "text-align": "left", 
                    ...
                    "nav-link-selected": {
                        "background": "linear-gradient(135deg, rgba(124, 58, 236, 0.4), rgba(0, 210, 255, 0.15))", 
                        "color": "white", 
                        "font-weight": "600",
                        "border-left": "4px solid #7c3aec",
            ...
            if selected:
                cleaned_selection = selected.lower().replace(" ", "_").strip()
                if cleaned_selection != current_page:
                    st.session_state.page = cleaned_selection
                    st.rerun()
        ...
        # Create a robust mapping of cleaned page names to original names
        page_mapping = {}
        for name in self.pages.keys():
            clean_key = re.sub(r'[^\x00-\x7F]+', '', name).strip().lower().replace(" ", "_")
            page_mapping[clean_key] = name
